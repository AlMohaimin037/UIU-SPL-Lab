...
int main()
{
    ...
    while(contFlag == 'y')
    {
        ...
        printf("\n\nYour choice:\t");
        scanf("%d", &opt);
        fgetc(stdin); // To consume the newline at the end of scanf
        ...
        switch(opt)
        {
            case 1:
                ...
                ord.customer[strlen(ord.customer)-1] = 0; // To vanish the newline which will come after fgets
                strcpy(ord.date, __DATE__); // Used for current date
                printf("\nPlease enter the number of items:\t");
                scanf("%d", &ord.numOfItems);
                for(int i=1; i<=ord.numOfItems; i++)
                {
                    fgetc(stdin);
                    printf("\n\n");
...
void generateBillFooter(float total)
{
    ...
    float grandTotal = netTotal + vat + gst;
    printf("----------------------------------------\n");
